...
通过 `n8n-open-cli` 命令行工具管理 self-hosted n8n 实例。该 CLI 默认 AI 友好模式，输出格式如下：

```jsonc
// 列表查询 → JSONL 格式（每行一个 JSON 对象，适合大量数据和流式处理）
{"id":"1","name":"Workflow A","active":true,...}
{"id":"2","name":"Workflow B","active":false,...}
{"__cursor":"MTIzZTQ1..."}   // 仅当有下一页时出现
...
列表命令（`workflow list`、`execution list`、`user list`、`tag list`、`variable list`、`project list`）输出 JSONL 格式：每行一个 JSON 对象，直接是数据本身（无 `ok`/`data` 包装）。

```bash
n8n-cli workflow list --fields id,name,active
# 输出：
# {"id":"1","name":"Workflow A","active":true}
# {"id":"2","name":"Workflow B","active":false}
# {"__cursor":"MTIzZTQ1..."}
...
解析方式：逐行 `JSON.parse()`。如果最后一行包含 `__cursor` 字段，表示还有下一页。

### 单条记录/写操作 → JSON envelope
...
检查顶层 `ok` 字段：
- `ok === true` → 操作成功，数据在 `data` 字段
- `ok === false` → 操作失败，错误信息在 `error` 字段，HTTP 状态码在 `code` 字段
...
- 列表命令输出 JSONL，如果最后一行是 `{"__cursor":"..."}` 则表示有下一页
- 将 `__cursor` 的值传给 `--cursor` 参数获取下一页
- 告知用户还有更多数据，询问是否继续获取
- 使用 `--limit` 控制每次返回数量
